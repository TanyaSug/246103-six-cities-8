...
const mapDispatchToProps = (dispatch: ThunkAppDispatch) => ({
  getNearByOffers: (offerId: number | string, setErrorValue: (message: string) => void) =>  dispatch(getNearByOffersAction(offerId, setErrorValue)),
  getReviews: (offerId: number | string, setErrorValue: (message: string) => void) =>  dispatch(getReviewsAction(offerId, setErrorValue)),
...
function OfferDetails(props: PropsFromRedux): JSX.Element | null {
  ...
  const [errorReviewMessage, setErrorReviewMessage] = useState<string>('');
  const [errorNearByMessage, setErrorNearByMessage] = useState<string>('');
  ...
  return (
    <div className="page">
      ...
      <main className="page__main page__main--property">
        {(!offersList.length || !offer) ? <div style={{color: 'red', padding: '16px'}}>Offer not found</div> :
          <>
              ...
              <div className="property__container container">
                <div className="property__wrapper">
                  ...
                  <section className="property__reviews reviews">
                    {errorReviewMessage
                      ? <div style={{color: 'red', padding: '16px'}}>Reviews: {errorReviewMessage}</div>
                      : <ReviewComponent offer={offer} />}
                  </section>
                </div>
              </div>
            ...
            {errorNearByMessage
              ? <div style={{color: 'red', padding: '16px'}}>Neighbourhood offers: {errorNearByMessage}</div>
              :
